...
#if !defined(__APPLE__) && !defined(__rtthread__) &&!defined(__ANDROID__)
...
#define netx_sockaddr_set_port( so_addr, port ) do{ if( (so_addr)){ if(((so_addr)->addr.ss_family == AF_INET)){(so_addr)->addr_v4.sin_port = htons(port);}else{(so_addr)->addr_v6.sin6_port = htons(port);}}}while(0)
...
extern char *netx_get_addrinfo( char *ip, char *port, int ai_flag, int ai_family, int ai_sock, int ai_proto );
extern char *netx_get_ifaddr();
